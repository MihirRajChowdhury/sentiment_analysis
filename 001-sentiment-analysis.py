...
# # LLM
# llm = ChatGroq(temperature=0, model_name="llama-3.1-8b-instant").with_structured_output(
#     Classification
# )
...
class Classification(BaseModel):
    sentiment:str = Field(description="The sentiment of the audience",enum=["hard","soft","enjoy"])
    political_tendency:str = Field(description="The political tendency of the user",enum=["With trump","With Biden","With No one"])
    language:str = Field(description="The language text is written in ",enum=["Hindi","Urdu"])

llm = ChatGroq(temperature=0,model="llama-3.1-8b-instant").with_structured_output(schema=Classification)
tagging_chain = tagging_prompt | llm
